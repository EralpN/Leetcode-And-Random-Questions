			...
			for (int i = 0; i < 3; i++) {
				...
				if (player.trim().toLowerCase().equals("stop")) {
					if (computerScore > playerScore) {
						System.out.println(
								"\tSo you have decided to give up...\n\tUnderstandable not many can win againts the mighty computer!");
						game.close();
						System.exit(0);
					} else if (computerScore == playerScore) {
						System.out.println(
								"\tWe should have played more there isnt even a clear winner...\n\tAnyway, play again later, bye!");
						game.close();
						System.exit(0);
					} else {
						System.out.println(
								"\tYou are a worthy challenger, your score is ahead of me...\n\tI will gladly play again with you, bye for now!");
						game.close();
						System.exit(0);
					}
				}
				...
				if ((player + computer).equals("rockrock") || (player + computer).equals("scissorsscissors")
						|| (player + computer).equals("paperpaper")) {
					condition = "draw";
				} else if ((player + computer).equals("rockscissors") || (player + computer).equals("scissorspaper")
						|| (player + computer).equals("paperrock")) {
					condition = "win";
				} else if ((player + computer).equals("rockpaper") || (player + computer).equals("scissorsrock")
						|| (player + computer).equals("paperscissors")) {
					condition = "lose";
				} else {
					System.exit(0);
				}
